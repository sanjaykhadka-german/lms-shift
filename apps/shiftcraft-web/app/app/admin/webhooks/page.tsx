...
const STATUS_BADGE: Record<string, string> = {
  succeeded: "bg-[var(--live)] text-white",
            ...
            {subscriptions.map((s) => {
              ...
              return (
                <li key={s.id} className="space-y-2 px-5 py-4">
                  ...
                  {revealed && (
                    <div className="rounded-md border border-[color-mix(in_srgb,var(--warn)_45%,transparent)] bg-[color-mix(in_srgb,var(--warn)_12%,transparent)] px-3 py-2 text-xs">
                ...
                {deliveries.map((d) => (
                  <tr key={d.id}>
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider ${STATUS_BADGE[d.status] ?? "bg-[var(--ink-3)] text-white"}`}
                      >
                        {d.status}
