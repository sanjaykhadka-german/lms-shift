...
- ✅ **Wage-budget guardrail per day** — `sc_locations.daily_wage_budget` numeric(10,2), nullable (null = guardrail off). Admin sets it on the location create/edit form; the list page badges sites that have one. The auto-scheduler (`lib/auto-scheduler.ts`) takes a per-(location, start-day) budget map and leaves a shift unfilled once every affordable candidate would push that day's projected wages over the cap (cheapest-first walk; rate-less candidates count as $0 and slot in as the in-budget fallback). Existing rostered shifts are seeded into the day's running wage so the cap reflects the whole roster, not just freshly-proposed shifts. The schedule's "Projected labour cost" card surfaces it three ways: a weekly total-vs-budget bar, a Mon–Sun daily-spend grid (amber within 10%, rose over), and per-location over/under chips. Migrations: public template `0032_red_proteus`, per-tenant `0044_shiftcraft_location_wage_budget` (both idempotent). Budget tests in `tests/auto-scheduler.test.ts`. A single daily figure applies to every weekday in v1; per-weekday budgets are a documented future refinement.
...
- ✅ **Idempotency** — `sc_xero_pay_runs` ledger unique on (tenant, week_start). The export idempotency key is a SHA-256 hash of the timesheet payload (`lib/payroll/idempotency.ts`): re-clicking with unchanged hours reuses the same key so Xero dedupes (no double-pay), while corrected hours yield a fresh key so the re-export goes through. Covered by `tests/xero-idempotency.test.ts`. CSV export at `app/api/timesheets/export/route.ts` remains as a fallback.
- ✅ **Pre-flight pay-calendar gate** — export fails fast if the connected Xero org has no pay calendar, and skips (with a named reason) any linked employee whose Xero record has no pay calendar — the two things Xero AU requires before a timesheet is accepted. (`listPayCalendars` + per-employee calendar check in `xero-actions.ts`.) The AU `Timesheet` model carries no `payrollCalendarID` of its own — the calendar lives on the employee record — so the push payload is unchanged.
...
- ✅ **Overtime-on-a-penalty-day at a separate rate** (2026-05-30) — added opt-in combo categories `penalty_sat_ot` / `penalty_sun_ot` / `penalty_ph_ot`. `buildCategoryUnitsFromBreakdown` now takes the tenant's mapped-category set: on a penalty day, ordinary minutes go to the base penalty bucket and OT minutes split into the matching `*_ot` combo **only when that combo is mapped to a Xero earnings rate** — otherwise OT folds into the base bucket exactly as before, so existing exports never break and no "missing mapping" error appears. The mapping admin UI lists the combos (with an "optional" note); migrations widen the `sc_xero_earnings_mapping` category CHECK (public `0033_cynical_mastermind`, per-tenant `0045_shiftcraft_penalty_ot_categories`, both idempotent). Tests in `tests/payroll-categories.test.ts` cover split-when-mapped + fold-when-unmapped. (The cost engine already priced OT-on-penalty via `bandMultiplier`; this slice was purely about the Xero export bucketing.)
